...
plt.title("Cohen's Kappa between Evaluators (Final order only - strict)")
# plt.xlabel("Evaluator")
# plt.ylabel("Evaluator")
...
# Use Gemini-2.5-Flash evaluation results
...
# Plot Root correct? vs. LEGIT score chart.
# 2-colored histogram
# Use Gemini-2.5-Flash evaluation results
generator_correct_legitscores = {}
for j, generator in enumerate(generator_list):
    generator_correct_legitscores[generator] = {"correct": [], "incorrect": []}
    for doc_id in evaluation_dict["gemini-2.5-flash"][generator].keys():
        root_correct = evaluation_dict["gemini-2.5-flash"][generator][doc_id]["evaluation_result"].get("", {}).get("correct_conclusion", False)
        score = evaluation_dict["gemini-2.5-flash"][generator][doc_id]["score"]
        generator_correct_legitscores[generator]["correct" if root_correct else "incorrect"].append(score)

# Draw stacked histogram. Correct lies on top of incorrect
for generator in generator_list:
    plt.figure(figsize=(8, 6))
    plt.hist([generator_correct_legitscores[generator]["correct"], generator_correct_legitscores[generator]["incorrect"]], bins=np.arange(0, 10.5, 0.5)-0.25, stacked=True, color=["red", "green"], label=["Incorrect", "Correct"])
    plt.xlabel("LEGIT Score")
    plt.ylabel("Number of Instances")
    plt.title(f"Root Correctness vs. LEGIT Score for {generator} (Gemini-2.5-Flash Evaluation)")
    plt.legend()
    plt.savefig(f"plots/root_correctness_vs_legit_score_{generator}.svg")
    plt.close()