...
async def main(path = "results/reasoning_tasks_gemini2.0.jsonl"):
    ...
    for result in results:
        ...
        for issue in reasoning_task["issues"]:
            ...
            evaluate_raw = await generate(prompt=prompts["evaluate_issue_in_response"].format(
                response=response,
        ...
        result["score"] = score
        with open(path.replace(".jsonl", "_evaluated.jsonl"), "w", encoding="utf-8") as f:
            for result in results:
