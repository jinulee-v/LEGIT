...
class BaseModelMerger(ABC):
    ...
    def get_transformers_auto_model_class(self):
        ...
        if self.model_config.architectures:
    ...
    def save_hf_model_and_tokenizer(self, state_dict: dict[str, torch.Tensor]):
        ...
        with init_empty_weights():
            if hasattr(auto_model_class, "from_config"):
                model = auto_model_class.from_config(self.model_config, torch_dtype=torch.bfloat16)
            elif hasattr(auto_model_class, "_from_config"):
                model = auto_model_class._from_config(self.model_config, torch_dtype=torch.bfloat16)
        ...
        if model.config.model_type == "gemma3":
            tokenizer = hf_tokenizer("google/gemma-3-4b-it")
        else:
